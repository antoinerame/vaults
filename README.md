...
- afficher une fiche detaillee pour le vault courant (TVL, APY, frais, share price, composition des allocations),
...
Ensuite ouvrez <http://127.0.0.1:5000>. Choisissez le reseau, saisissez l'adresse du vault et les dates (ou cliquez sur les boutons 7 j / 30 j / ...) ou utilisez `TOUT` pour l'historique complet. La section "Curator" permet de retrouver les vaults d'un curator (slug ou adresse) et de charger un vault en un clic. La fiche "Vault actuel" recapitule TVL/APY/frais/composition et un bouton permet d'ouvrir la page `https://app.morpho.org/{network}/vault/{address}` dans un nouvel onglet.
